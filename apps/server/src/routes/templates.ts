...
const createTemplateSchema = z.object({
  ...
  category: z.enum(['maintenance', 'inspection', 'service', 'installation', 'calibration', 'breakdown', 'custom']),
...
const searchTemplatesSchema = z.object({
  ...
  category: z.enum(['maintenance', 'inspection', 'service', 'installation', 'calibration', 'breakdown', 'custom']).optional(),
...
router.get('/', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const { page = 1, limit = 20, category, status, search, sortBy = 'updatedAt', sortOrder = 'desc' } = query;
...
router.post('/', authorize('admin', 'manager'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
...
router.delete('/:id', authorize('admin'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const templateId = req.params.id;
