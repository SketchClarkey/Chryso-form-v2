...
router.post('/reports/:id/schedule', authenticate, [
...
router.delete('/reports/:id/schedule', authenticate, [
  param('id').isMongoId().withMessage('Invalid report ID'),
], async (req, res) => {
  try {
    const errors = validationResult(req);
...
router.put('/jobs/:id/pause', authenticate, [
...
router.put('/jobs/:id/resume', authenticate, [
  param('id').isString().withMessage('Invalid job ID'),
], async (req, res) => {
  try {
    const errors = validationResult(req);
...
router.post('/email/test', authenticate, [
...
router.put('/email/config', authenticate, [
