...
import { authenticate, authorize, AuthenticatedRequest, requireWorksite } from '../middleware/auth.js';
...
router.get('/', auditDataAccess, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const { page, limit, sort, order, q, status, worksite, technician, dateFrom, dateTo } = searchSchema.parse(req.query);
...
router.get('/:id', async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    if (currentUser.role === 'manager' && !currentUser.worksiteIds.includes(form.worksite._id.toString())) {
      res.status(403).json({
        success: false,
...
// Create new form
router.post('/', requireWorksite, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const currentUser = req.user!;
...
router.post('/:id/review', authorize('admin', 'manager'), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    const formId = req.params.id;
    const currentUser = req.user!;
