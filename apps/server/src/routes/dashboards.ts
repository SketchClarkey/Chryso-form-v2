...
const validateWidget = [
  ...
  body('config')
    .isObject()
    .withMessage('Widget configuration is required'),
  body('layout')
    .isObject()
    .withMessage('Widget layout is required'),
];
...
router.get('/:id', authenticate, [
  ...
  try {
    ...
    const status = error.message === 'Dashboard not found' ? 404 : 
...
router.post('/', authenticate, validateDashboard, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({
...
router.delete('/:id', authenticate, [
  ...
  try {
    ...
    await dashboardService.deleteDashboard(req.params.id, req.user?.role || 'technician', req.user?.id || '');
...
router.post('/:id/duplicate', authenticate, [
  param('id').isMongoId().withMessage('Invalid dashboard ID'),
...
router.get('/:id/data', authenticate, [
  param('id').isMongoId().withMessage('Invalid dashboard ID'),
...
router.post('/:id/widgets', authenticate, [
  param('id').isMongoId().withMessage('Invalid dashboard ID'),
  ...validateWidget,
], async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.put('/:id/widgets/:widgetId', authenticate, [
  param('id').isMongoId().withMessage('Invalid dashboard ID'),
...
router.delete('/:id/widgets/:widgetId', authenticate, [
  param('id').isMongoId().withMessage('Invalid dashboard ID'),
  param('widgetId').isString().withMessage('Invalid widget ID'),
], async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/templates/list', authenticate, [
...
router.post('/templates/:id/create', authenticate, [
