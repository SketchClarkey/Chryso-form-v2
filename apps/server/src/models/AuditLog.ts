...
export interface IAuditLog extends Document {
  ...
  // Event Information
  eventType: 'create' | 'read' | 'update' | 'delete' | 'login' | 'logout' | 'access' | 'export' | 'import' | 'admin' | 'system';
  action: string; // Specific action taken (e.g., 'form_created', 'user_login', 'data_exported')
  category: 'authentication' | 'data' | 'user_management' | 'system' | 'security' | 'compliance' | 'integration';
...
interface IAuditLogModel extends Model<IAuditLog> {
  ...
  getComplianceReport(organizationId: string, complianceType?: string, startDate?: Date, endDate?: Date): any;
...
const auditLogSchema = new Schema<IAuditLog>({
  ...
  eventType: { 
    type: String, 
    enum: ['create', 'read', 'update', 'delete', 'login', 'logout', 'access', 'export', 'import', 'admin', 'system'],
    required: true
  },
  action: { type: String, required: true },
  category: { 
    type: String, 
    enum: ['authentication', 'data', 'user_management', 'system', 'security', 'compliance', 'integration'],
    required: true
  },
...
// Static methods
auditLogSchema.statics.getByUser = function(organizationId: string, userId: string, startDate?: Date, endDate?: Date) {
...
auditLogSchema.statics.getByResource = function(organizationId: string, resourceType: string, resourceId: string) {
  return this.find({ organizationId, resourceType, resourceId }).sort({ timestamp: -1 });
};

auditLogSchema.statics.getSecurityEvents = function(organizationId: string, startDate?: Date, endDate?: Date) {
  const query: any = { 
    organizationId,
    $or: [
      { category: 'security' },
...
auditLogSchema.statics.getAnomalousActivity = function(organizationId: string, hours: number = 24) {
