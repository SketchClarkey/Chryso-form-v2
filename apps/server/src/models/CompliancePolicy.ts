...
export interface ICompliancePolicy extends Document {
  ...
  rules: Array<{
    ...
    name: string;
    description: string;
    category: 'data_handling' | 'access_control' | 'retention' | 'encryption' | 'audit' | 'notification';
...
const compliancePolicySchema = new Schema<ICompliancePolicy>({
  ...
  rules: [{
    ...
    category: { 
      type: String, 
      enum: ['data_handling', 'access_control', 'retention', 'encryption', 'audit', 'notification'],
      required: true
    },
    ruleType: { 
      type: String, 
  ...
  auditRequirements: {
    ...
    internalAudits: {
      enabled: { type: Boolean, default: false },
      frequency: { type: String, enum: ['monthly', 'quarterly', 'annually'], default: 'quarterly' },
      scope: [String]
    },
...
compliancePolicySchema.methods.evaluateCompliance = function(auditLogs: any[]) {
  ...
  this.rules.forEach((rule: any) => {
    ...
    const ruleViolations = auditLogs.filter(log => {
      if (rule.conditions.resourceTypes && !rule.conditions.resourceTypes.includes(log.resourceType)) {
        return false;
      }
...
compliancePolicySchema.methods.getApplicableRules = function(context: any) {
  return this.rules.filter((rule: any) => {
    ...
    if (rule.conditions.resourceTypes && context.resourceType && 
        !rule.conditions.resourceTypes.includes(context.resourceType)) {
      return false;
    }
    
    if (rule.conditions.actions && context.action && 
