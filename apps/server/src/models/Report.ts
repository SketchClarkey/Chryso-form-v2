...
reportSchema.methods.exportReport = async function(format: 'pdf' | 'excel' | 'csv'): Promise<Buffer> {
...
reportSchema.methods.clone = async function(newName: string, userId: Types.ObjectId): Promise<IReport> {
  const cloneData = {
    name: newName,
