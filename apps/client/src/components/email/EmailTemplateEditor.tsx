...
const EmailTemplateEditor: React.FC<EmailTemplateEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    try {
      const url = template ? `/api/email-templates/${template._id}` : '/api/email-templates';
  ...
  const handlebarsHelpers = [
    ...
    {
      name: 'Format Date',
      syntax: '{{formatDate dateVariable "YYYY-MM-DD"}}',
      description: 'Format a date variable',
    },
    {
      name: 'Currency',
      syntax: '{{currency amount "USD"}}',
  ...
  const availableTypes = types.filter(
    type => formData.category === 'custom' || type.category === formData.category
  );
