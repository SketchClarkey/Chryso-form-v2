  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const processedMetrics: SecurityMetrics = {
        ...
        failedLogins:
          summary
            .find((cat: any) => cat._id === 'authentication')
            ?.events?.find((event: any) => event.status === 'failure')?.count || 0,
        criticalEvents: summary.reduce(
          (acc: number, cat: any) =>
            acc + (cat.events?.filter((e: any) => e.severity === 'critical')?.length || 0),
          0
        ),
        complianceViolations:
          summary.find((cat: any) => cat._id === 'compliance')?.totalEvents || 0,
        activeUsers: new Set(alertsRes.data.alerts.map((a: any) => a.userEmail).filter(Boolean))
          .size,
        trends: {
    ...
    <Box p={3}>
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          ...
                          {
                            name: 'Access Control',
                            value: Math.floor(metrics.securityAlerts * 0.3),
                          },
                ...
                <CardContent>
                  <Box display='flex' justifyContent='space-between' alignItems='center' mb={2}>
                    ...
                    <Chip
                      ...
                      color={
                        alerts.length > 10 ? 'error' : alerts.length > 5 ? 'warning' : 'success'
                      }
                      size='small'
                    />
                  </Box>
      ...
      <Card>
        <CardContent>
          ...
          <AuditLogViewer organizationId={organizationId} height={500} embedded />
        </CardContent>
      </Card>
    </Box>
  );
